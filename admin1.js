...
async function deleteProduct(productId) {
 ...
 function bindUpdateButton(productId) {
  ...
  const color_id = parseInt(block.querySelector(".colorSelect").value);
  const age_range_id = parseInt(block.querySelector(".age_range").value); // Add a dropdown for age ranges
...
async function updateProduct(productId) {
  ...
  addInventoryBlock(colors, item); // This should pass the full object
});
...
/*function addInventoryBlock(colors, item, index) {
  const colorsContainer = document.getElementById("inventoryContainer");

  // Create inventory block HTML
  let inventoryHTML = `
    <h5>Inventory ${index + 1}</h5>
  ...
  colors.forEach((color) => {
    inventoryHTML += `<option value="${color.id}" ${item.color_id === color.id ? "selected" : ""}>${color.name}</option>`;
  });

  inventoryHTML += `</select><br>
    <label for="quantity_${index}">Quantity:</label>
    <input type="number" id="quantity_${index}" name="quantity_${index}" value="${item.quantity}"><br>`;

  // Check if age range exists and populate
  if (item.age_range) {
    inventoryHTML += `
      <h6>Age Range</h6>
      <label for="min_value_${index}">Min Value:</label>
      <input type="number" id="min_value_${index}" name="min_value_${index}" value="${item.age_range.min || 0}"><br>
      <label for="max_value_${index}">Max Value:</label>
      <input type="number" id="max_value_${index}" name="max_value_${index}" value="${item.age_range.max || 0}"><br>
      <label for="unit_${index}">Unit:</label>
      <input type="text" id="unit_${index}" name="unit_${index}" value="${item.age_range.unit || 'N/A'}"><br>`;
  } else {
    inventoryHTML += `
      <h6>Age Range</h6>
      <label for="min_value_${index}">Min Value:</label>
...
const unit = ageRange?.unit ?? "years";  // Use 'unit' from 'age_range'
  const ageQuantity = quantity ?? "";
