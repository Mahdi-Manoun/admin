...
function bindUpdateButton(productId) {
  ...
  updateBtn.onclick = async function () {
    ...
    inventoryInputs.forEach((_, index) => {
      const colorId = parseInt(document.getElementById(`color_id_${index}`).value);
      const quantity = parseInt(document.getElementById(`quantity_${index}`).value);
     const min = parseInt(document.getElementById(`min_value_${index}`).value);
const max = parseInt(document.getElementById(`max_value_${index}`).value);
let unit = document.getElementById(`unit_${index}`).value;

if (!unit || unit.trim() === "") {
  unit = "N/A";
}
const hasValidAgeRange = !isNaN(min) || !isNaN(max) || unit !== "N/A";
...
product.inventory.push(inventoryItem);
    });
...
async function updateProduct(productId) {
  ...
  inventory.forEach((inventoryItem, index) => {
    const colorId = inventoryItem.color?.id ?? 0;
    const quantity = inventoryItem.quantity ?? 0;

    // Handle null age_range
    const ageRange = inventoryItem.age_range || {};
    const min = ageRange.min != null ? ageRange.min : 0;
    const max = ageRange.max != null ? ageRange.max : 0;
    const unit = ageRange.unit != null ? ageRange.unit : 'N/A';
    ...
    colors.forEach((color) => {
      const selected = color.id === colorId ? "selected" : "";
      inventoryHTML += `<option value="${color.id}" ${selected}>${color.name}</option>`;
    });

    inventoryHTML += `</select><br>

      <label for="quantity_${index}">Quantity:</label>
      <input type="number" id="quantity_${index}" name="quantity_${index}" value="${quantity}"><br>

      <h6>Age Range</h6>
      <label for="min_value_${index}">Min Value:</label>
      <input type="number" id="min_value_${index}" name="min_value_${index}" value="${min}"><br>

      <label for="max_value_${index}">Max Value:</label>
      <input type="number" id="max_value_${index}" name="max_value_${index}" value="${max}"><br>

      <label for="unit_${index}">Unit:</label>
      <input type="text" id="unit_${index}" name="unit_${index}" value="${unit}"><br>
    `;
    ...
    if (images.length > 0) {
      images.forEach((imageItem, index) => {
        const imageHTML = `
          ...
          <input type="file" id="url_${index}" name="url_${index}" value="${imageItem.url}"><br>
        `;
